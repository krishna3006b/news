...
const Article = () => {
  const { id } = useParams<{ id: string }>(); // e.g., "QmUhhqYeW4CCLFqhx5SpFwfBpyUZxBThVRSviyuGmWxhda/0"
  ...
  useEffect(() => {
    ...
    const fetchArticle = async () => {
      setLoading(true);
      try {
        // Reconstruct the full IPFS hash for display
        const fullIpfsHash = `ipfs://${id}`;
        setIpfsHash(fullIpfsHash);

        console.log("Fetching IPFS content for hash:", id);
        const response = await fetch(`https://ipfs.io/ipfs/${id}/0`); // Use the cleaned id directly
        if (!response.ok) {
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <code className="bg-muted px-2 py-1 rounded text-sm font-mono break-all">
                        {ipfsHash} {/* Display the full hash with ipfs:// */}
                      </code>
                      <a
                        href={`https://ipfs.io/ipfs/${id}/0`} // Link uses the cleaned id
                        target="_blank"
